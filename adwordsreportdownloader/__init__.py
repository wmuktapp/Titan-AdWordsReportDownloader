...
class FlowManager(object):
    ...
    def __init__(self, report_config, account_ids, ftp_destination, ftp_file_name):
        ...
        Positional Arguments:
        1. report_config (dict): The serialised JSON-structured AdWords report configuration. For more details, see
        https://developers.google.com/adwords/api/docs/guides/reporting#create_a_report_definition
        2. account_ids (list): A list of AdWords Account IDs (aka client customer IDs) to request reports for
        3. ftp_destination (tuple): FTP connection details in the format of (protocol, user, pass, host, port, path) -
        ...
        4. ftp_file_name (string): The name of the file to write to FTP. If --ftp-destination is not provided, this is
        ignored
        5. disable_ssl_verification (bool): Whether or not SSL verification should be disabled (defaults to False)
    ...
    def get_report_streams(self):
        ...
        streams = []
        client = adwords.AdWordsClient.LoadFromStorage(os.path.join(self.THIS_DIRECTORY, "googleads.yaml"))
        ...
        client.soap_impl = "suds"
        downloader = client.GetReportDownloader(version="v201802")
...
class TitanFlowManager(FlowManager):
    def __init__(self, report_config, account_ids):
        """Initialise an object that controls the flow of the application - from chaining the API calls to upload the
        ...
        https://developers.google.com/adwords/api/docs/guides/reporting#create_a_report_definition
        2. account_ids (list): A list of AdWords Account IDs (aka client customer IDs) to request reports for
        3. disable_ssl_verification (bool): Whether or not SSL verification should be disabled (defaults to False)
...
def main(report_string, account_ids, ftp_destination, ftp_file_name, load_date, previous_days):
    """Download an AdWords report based on a JSON input and upload directly to either FTP or Titan's blob storage.

    Positional Arguments:
    1. report_string (string): The JSON-structured AdWords report configuration. For more details, see
    https://developers.google.com/adwords/api/docs/guides/reporting#create_a_report_definition
    2. account_ids (string): A space delimited string of AdWords Account IDs (aka client customer IDs) to request
    reports for
    3. ftp_destination (string): FTP connection details in the format, ftp|ftps://<user>:<pass>@<host>[:port][/path].
    If this is not provided, it is assumed that the execution environment is Titan in which case, the file will be
    uploaded to blob storage
    4. ftp_file_name (tuple): The name of the file to write to FTP. If --ftp-destination is not provided, this is
    ignored
    5. load_date (datetime.date): The end date that the report date range should reference. This is only added to the
    JSON if there is no dateRangeType present
    6. previous_days (int): The amount of days to fetch data for. The start date is equal to
